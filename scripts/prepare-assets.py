...
Derived from chuspeeism/iphone-duo's prepare-assets.py (MIT). This variant keeps the
same "Landscape" (fully unfolded) pose as the base mesh — the two custom poses in
this app (Tent, StandBy) are produced client-side in main.js by rolling and folding
that same base mesh, not by selecting a different Pose variant (the model only
ships "Closed" and "Landscape").
"""
